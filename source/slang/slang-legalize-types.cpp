...
bool isLogicalAddressSpace(AddressSpace addrSpace)
{
    ...
    // other SPIR-V storage class is logical, and a logical pointer may not sit inside a composite.
...
bool isLogicalPointerType(IRType* type)
{
    if (auto ptrType = as<IRPtrTypeBase>(unwrapAttributedType(type)))
    {
        ...
        // physical, so we treat it as logical to preserve the composite-membership restriction.
